/*
	...
	* 
	* edited by J.Q on 01/06/2017.
	* This version need PAPI.
*/
...
#include "seq.cpp"
#include "PapiInitialization.hpp"
...
int main(int argv, char* argc[])
{
	...
	papitestfile=argc[11];
	papifill(papitestfile);
	...
	{
		if(papi_test_n[NUM_CORES-1]==false)
			continue;
		...
		for (repeat=0;repeat<1;repeat++)	// The repeat time can be changed if needed
		{
	    	...
	    	//-----------------------------
		//PAPI
	    	papithread=NUM_THREADS;
	    	if(Papi_Initialization())
	    		return -1;
		//------------------------------
			...
			if (mode==0||NUM_THREADS==1)			// mode=0 means only checking seq
			{
				...
				// PAPI 
				cout<<"------------------"<<endl;
				pstart_time=PAPI_get_real_nsec();		// When everything starts (global)
	       			pbefore_time=PAPI_get_real_nsec();		// Before running the program (global)
	        		retval = PAPI_start(EventSet);			// Let PAPI starts (global)
				//------------------------------
				...
				//------------------------------
				// PAPI
	        		pafter_time=PAPI_get_real_nsec();		// Count the end time
	        		retval = PAPI_stop(EventSet, values);	// Let PAPI stop
	        		ptotal_time=((double)(pafter_time-pstart_time))/1.0e9;
	        		pelapsed_time=((double)(pafter_time-pbefore_time))/1.0e9;
	       			cout<<endl<<"etotal_time is "<<ptotal_time<<endl;
	       			cout<<"elapsed_time is "<<pelapsed_time<<endl;
				Papicalculation();
				cout<<"----------------------------------"<<endl;
				//------------------------------
			...
			if (mode==1&&NUM_THREADS>1)
			{
				...
				//------------------------------
				// PAPI 
				pstart_time=PAPI_get_real_nsec();
	        		pbefore_time=PAPI_get_real_nsec();
				...
				// PAPI
	        		/* Stop Counting */
	        		pafter_time=PAPI_get_real_nsec();
